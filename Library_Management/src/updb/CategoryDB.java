...
public class CategoryDB 
{
    ...
    public static void fixCategory() throws SQLException 
    {
        String sql = "UPDATE categories c JOIN ( "
                + "SELECT category , SUM(quantity) as total FROM books GROUP BY category )" 
                +"b ON c.category_name = b.category SET c.totalbook = b.total;";
        try (Connection conn = DBConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) 
        {            
            stmt.executeUpdate();
        }
